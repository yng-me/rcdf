...
test_that("read_dot_env skips comments, empty lines, and other special casees", {
  ...
  writeLines(
    c(
      ...
      " ",
      "DB_PW='sdf=dfdfa=xcd'",
      "DB_PW_2=sdf=dfdfa=xcd",
      'DB_PW_3="sdf=dfdfa=xcd"',
      "DB_NAME='db' # Test DB"
    ),
    temp_env_file
  )
  ...
  expect_equal(env_vars$DB_HOST, "localhost")
  expect_equal(env_vars$DB_USER, "root")
  expect_equal(env_vars$DB_PW, "sdf=dfdfa=xcd")
  expect_equal(env_vars$DB_PW_2, "sdf=dfdfa=xcd")
  expect_equal(env_vars$DB_PW_3, "sdf=dfdfa=xcd")
  expect_equal(env_vars$DB_NAME, "'db' # Test DB")

  # Clean up temporary file
  unlink(temp_env_file)
})

test_that("read_dot_env returns NULL if no variables are found", {
...
test_that("read_dot_env defaults to .env in current directory if no path is provided", {
  ...
  # Check that read_dot_env can read the file without specifying path
  setwd(dir_temp)
...
test_that("read_dot_env throws an error when the file doesn't exist", {
